/*
 ================================================================
  ...
  DOM généré par addMsg :
    .chat-msg
      .chat-avatar   [border colorée = couleur pseudo]
      .chat-bar      [barre verticale colorée]
      .chat-body
        .chat-topline  [pseudo + badges — à cheval sur le haut de la bulle]
        .chat-bubble
          .chat-text
 ================================================================
*/
...
function renderEmotes(text, emotes) {
  ...
  while (i < chars.length) {
    if (map.has(i)) { const en=map.get(i); out+=en.html; i=en.end+1; } else out+=esc(chars[i++]);
  }
  return out;
}
...
function buildAvatar(name, imgUrl, color) {
  ...
  // border colorée = couleur du pseudo
...
function renderText(data, isTest) {
  ...
  if (isTest) {
    ...
    let out=esc(rawText);
    testEmotes.forEach(([name,id])=>{ out=out.replace(new RegExp(`\\b${escRx(name)}\\b`,'g'),`<img class="emote" src="https://static-cdn.jtvnw.net/emoticons/v2/${id}/default/dark/3.0" alt="${name}" onerror="this.remove()">`); });
    return out;
  }
...
// ── addMsg ────────────────────────────────────────────────────
// DOM : .chat-msg
//   > .chat-avatar  (border = couleur pseudo)
//   > .chat-bar     (bg = couleur pseudo)
//   > .chat-body
//       > .chat-topline  (pseudo + badges, chevauche le haut de la bulle)
//       > .chat-bubble
//           > .chat-text
